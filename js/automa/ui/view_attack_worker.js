...
viewModelList.push(function () {
	...
	currentView.subscribe(function (newValue) {
		...
		if (GAME && newValue === vm.viewName) {			
			var hexs = hexConflict.workers;
			var baseIndex,p1, p2, num_of_Worker = 0,
				num_territories = hexs.length;
				hexs.forEach(function (hex_attack, i) {
				if (i === 0) {
			...
			if (num_territories > 1) {
				var textTerritories = _i('(in % territories)');
				i_textTerritories = textTerritories.replace('%', num_territories);
			}
			vm.txt_territories(i_textTerritories);
